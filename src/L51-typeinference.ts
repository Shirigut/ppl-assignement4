...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => {
    const tenv: E.TEnv = E.makeEmptyTEnv();
    const classes: T.ClassTExp[] = R.map(A.classExpToClassTExp, A.parsedToClassExps(parsed)); 
    const typenames: string[] = R.map(x=> x.typename, classes);
    return E.makeExtendTEnv(typenames, classes, tenv);
}
...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    ...
    const value = typeofExp(exp.val, extEnv);
    const equals: Result<true> = bind(value, (value: T.TExp) => checkEqualType(value, exp.var.texp, exp));
    return bind(equals, () => makeOk(T.makeVoidTExp()));
}
...
const typeofProgramExps = (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp> => {
    if (A.isDefineExp(exp) && isEmpty(exps)) return typeofDefine(exp,tenv)
    if (isEmpty(exps)) return typeofExp(exp, tenv)
    if(A.isDefineExp(exp)) {
        const extEnv: E.TEnv= E.makeExtendTEnv([exp.var.var],[exp.var.texp], tenv)
        return bind (typeofExp(exp, tenv), () => typeofProgramExps(first(exps), rest(exps), extEnv))
    }
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    return safe2((param1: T.TExp, param2: T.TExp) => bind(checkEqualType(param1, param2, exp), () => makeOk(T.makeVoidTExp())))
    (typeofExp(exp.var, tenv), typeofExp(exp.val, tenv))
};
...
// type = classtexp 
//check if the methods are ok
//return proc exp that gets all the fields and returns classtexp(type name and all the methods)
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const fields: T.TExp[] = R.map(x => x.texp , exp.fields);
    const typenames: string[] = R.map(x=> x.var, exp.fields);
    const extenv = E.makeExtendTEnv(typenames, fields, tenv);
    const methodNames: T.TExp[] = R.map(x=>x.var.texp, exp.methods); // array of types
    const methods: Result<T.TExp[]> = mapResult(x => typeofExp(x.val, extenv), exp.methods);  //result array of types
    return safe2((v:T.TExp[], t) => bind(checkEqualTypes(v, methodNames, exp), () => makeOk(T.makeProcTExp(fields, A.classExpToClassTExp(exp)))))
    (methods, makeOk(methodNames))
