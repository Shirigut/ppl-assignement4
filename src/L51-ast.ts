...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> => {
    ...
    const varDeclsResult = mapResult(parseVarDecl, varDecls);
    if (!isString(typeName))
        return makeFailure(`Invalid TVar`);
    const tResult = makeOk(makeTVar(typeName.toString()));
    if (! isGoodBindings(bindings)) return makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`);
    else 
    return safe3((type: TVar, fields: VarDecl[], bdgs: Binding[]) => makeOk(makeClassExp(type ,fields, bdgs)))
    (tResult, varDeclsResult ,parseBindings(bindings));
}
...
export const parsedToClassExps = (p: Parsed): ClassExp[] => 
    isProgram(p)? chain(parsedToClassExps, p.exps):
    isClassExp (p) ? [p].concat(chain(parsedToClassExps , map((bdg:Binding) => bdg.val, p.methods))):
    isDefineExp(p)? chain(parsedToClassExps, [p.val]):
    isProcExp(p)? chain(parsedToClassExps, p.body):
    isIfExp(p)? chain(parsedToClassExps, [p.then, p.alt]):
    isLetExp(p)? chain(parsedToClassExps, p.body):
    isLetrecExp(p)? chain(parsedToClassExps, p.body):
    isAppExp(p)? chain(parsedToClassExps, p.rands.concat([p.rator])):
    isSetExp(p)? chain(parsedToClassExps, [p.var, p.val]):
    [];
